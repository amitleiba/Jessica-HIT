    ...
    MetricsDbContext context) : ControllerBase
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            // Safety >= 1 means an obstacle was detected (robot only sends 0=clear or 1=obstacle)
            var safetyIncidents = metrics.Count(m => m.Safety >= 1);
    ...
    public async Task<IActionResult> DeleteAll(CancellationToken cancellationToken = default)
    {
        try
        {
            var deleted = await _context.SensorMetrics.ExecuteDeleteAsync(cancellationToken).ConfigureAwait(false);
            _logger.LogInformation("Metrics DB cleared by admin. {Count} records deleted.", deleted);
            return Ok(new { message = $"Cleared {deleted} records." });
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error clearing metrics database");
            return StatusCode(500, new { message = "Internal server error while clearing metrics." });
        }
    }
